...
public non-sealed class WindowsAArch64VaList implements VaList, Scoped {
    ...
    private WindowsAArch64VaList(MemorySegment segment, MemorySession session) {
    ...
    static WindowsAArch64VaList ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment segment = MemorySegment.ofAddress(addr, Long.MAX_VALUE, session);
        return new WindowsAArch64VaList(segment, session);
    }
